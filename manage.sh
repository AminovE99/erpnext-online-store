...
down() {
    ...
    dc "${1}" down ${@:2}
    rm -rf "${ERPNEXT_HOME:-/srv/erpnext_online_store/frappe}"/*
}
...
#   - Replace all occurences of `erpnext_online_store` and `ERPNext Template` in all files
#   - Rename all directories `erpnext_online_store`
...
prepare_release() {
    ...
    sed -i \
        ...
        ./erpnext_online_store/__init__.py
